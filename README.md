...
| `ai-quality/` | AI 质量优化闭环（Phase 46）：统一 AI Feedback（8 渠道）/ Error Taxonomy + 聚类 / Improvement Proposal（离线评测 + Gate）/ Prompt·Model 版本管理 / A/B 对比 / 多目标评分 / Shadow·Canary·自动回滚 / Knowledge Learning·Quality·Decay / Continuous Evaluation·Benchmark 自动扩充·Change Impact·AI Release Gate·Audit / 状态持久化（persistToFile / loadFromFile） |
...
│   ├── ai-quality/              # ★ AI 质量优化闭环（v4.21.0/Phase 46）：feedback / error-analysis / improvement / versioning / experiment / knowledge-learning / ops / service（Feedback → 聚类 → 提案 → 离线评测 → 审批 → Shadow/Canary → 回滚 → 审计，持久化跨重启保留）
│   ├── web/                     # Web Dashboard 前端（React + Vite，v4.1/Phase 25.6；Vitest 单元/组件测试 v4.17.0/Phase 42.1 + 交互正确性/全生命周期 v4.18.0/Phase 43 + AI 质量/改进页 v4.21.0/Phase 46）
