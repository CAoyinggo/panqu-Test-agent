...
基于 Vitest（含 v8 覆盖率）。全量 `npm test` 共 **122 个测试文件、1445 条用例**（含旧用例，无回归）；`agent:test` 450 条（Phase 1-23 行为保持）。
