...
│   ├── platform/                # ★ AI Test Platform 平台层（v4.0-v4.18）：projects / storage / runs / scheduler / workers / rbac / approval-center / events / notifications / audit / auth / telemetry / operations / ops / service / api / security / test-assets / workflow
│   ├── web/                     # Web Dashboard 前端（React + Vite，v4.1/Phase 25.6；Vitest 单元/组件测试 v4.17.0/Phase 42.1 + 交互正确性/全生命周期 v4.18.0/Phase 43）
...
├── tests/                       # Vitest 测试：unit/（单元） + integration/ + e2e/（141 文件 / 1586 用例，全量回归全绿）+ tests/e2e/web/（14 个 Playwright 真实浏览器 E2E 套件，Chromium v4.16.0 / 跨浏览器 v4.17.0）
