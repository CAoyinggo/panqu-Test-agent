...
│   ├── ai-quality/              # ★ AI 质量优化闭环（v4.21.0/Phase 46 + v4.23.0/Phase 48 + v4.24.0/Phase 49）：feedback / error-analysis / improvement / versioning / experiment / knowledge-learning / ops / continuous-eval（Phase 48：Nightly·Weekly·Release 定时评测，回归判定 BLOCK→Alert+Block Release）/ eval-bridge（Phase 49：Eval→Feedback 桥接 + Benchmark 扩充候选 BenchmarkCandidateStore，PENDING_REVIEW 人工 Review 才可并入）/ service（Feedback → 聚类 → 提案 → 离线评测 → 审批 → Shadow/Canary → 回滚 → 审计 → 持续评测 → 失败桥接，持久化跨重启保留）
│   ├── web/                     # Web Dashboard 前端（React + Vite，v4.1/Phase 25.6；Vitest 单元/组件测试 v4.17.0/Phase 42.1 + 交互正确性/全生命周期 v4.18.0/Phase 43 + AI 质量/改进页 v4.21.0/Phase 46 + 持续评测 Tab v4.23.0/Phase 48 + Benchmark 扩充 Tab v4.24.0/Phase 49）
