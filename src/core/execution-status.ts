...
export type CoreExecutionStatus = 'PASS' | 'FAIL' | 'BLOCKED' | 'NOT_EXECUTED';
...
export function evaluateCoreExecution(input: ExecutionStateInput): CoreExecutionState {
  ...
  const passed = input.checks.filter((check) => check.pass).length;
  const passRate = Math.round((passed / input.checks.length) * 100);
  return passed === input.checks.length
    ? { executed: true, status: 'PASS', passRate }
    : { executed: true, status: 'FAIL', passRate, reason: `${input.checks.length - passed} 条断言失败` };
}
