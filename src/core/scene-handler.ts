...
import type { CanonicalSceneId } from './canonical-scene.js';

export interface SceneHandler {
  /** 处理器标识（如 'video'） */
  name: string;
  /** Processor 明确声明支持的 canonical scene ID。 */
  supportedScenes: readonly CanonicalSceneId[];
  /** 是否支持指定 canonical scene。 */
