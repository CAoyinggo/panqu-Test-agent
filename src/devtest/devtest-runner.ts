...
export async function runDevTest(options: DevTestOptions): Promise<DevTestRunResult> {
  ...
  const executionPlan = buildAcceptanceExecutionPlanIdentity({ markdown, allTestCases: preview.testCases, selectedCaseIds });
  if (options.expectedExecutionPlan) {
    const checked = validateAcceptanceExecutionPlanIdentity({ expected: options.expectedExecutionPlan,
