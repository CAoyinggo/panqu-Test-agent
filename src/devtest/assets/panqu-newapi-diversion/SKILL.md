...
- **【两套真源，别混】**：
  - **价格/成本对比** → 飞书《分流渠道表》快照 `references/feishu-live-pricing-cache.json`，判定模块 `src/devtest/diversion-pricing-authority.ts`（`resolveListPrice` 刊例价不变量 / `resolveChannelCost` 含 `J×折扣` / `rankChannelsByCost` 成本升序）。刷新：`python3 scripts/refresh-feishu-pricing.py`。
  - **运行时可分流资格**（模型 × 分辨率 × **画面比例** × 渠道启用）→ **NewAPI 网关渠道配置**（`apitest.panqu.com/channels`「模型与分组」），镜像在 `pq_aivideo_diversion_config`(line=10) `newapi_route_rules`；门槛与判定模块 `src/devtest/newapi-route-eligibility.ts`（`evaluateVideoDiversion`/`evaluateImageDiversion` 忠实镜像 `check_diversion`/`NewapiImageDiversionService`）。完整说明见 `references/newapi-eligibility-gate.md`。**⚠️ 资格以网关配置为准，不是飞书表；飞书表只管价格。**
