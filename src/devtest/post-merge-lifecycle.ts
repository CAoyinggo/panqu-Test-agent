...
export class PostMergeLifecycleEngine {
  ...
  public static async execute(input: PostMergeInput): Promise<PostMergeResult> {
    ...
    releaseLines.push(
      ...
      `- **零白嫖刊例核验**: ${driftReport.issues.some((i) => i.category === 'UNPRICED_MODEL') ? '⚠️ 存在未定价模型风险' : '✅ 评估模型在 FastAdmin 数据库刊例已配置完备'}；`,
      `- **账务不变量与退款防线**: ⚪ NOT_EXECUTED (合入后生命周期主要核查配置与毛利，未运行计费断言，需结合线上流水监控)。`,
      '',
      '---',
      '',
