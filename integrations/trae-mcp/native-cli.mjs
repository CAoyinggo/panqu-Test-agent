...
CLI_TOOL.inputSchema.properties.confirm_real_execution={type:'boolean',description:'只有用户明确授权本次真实 Playwright、生图、视频、画布或综合套件操作及费用后才可为 true；有效的 Mock/帮助模式不需要。'};
CLI_TOOL.description+=' program=playwright 暴露完整 CLI（先用 --help）。本地报告通过 panqu_report/panqu_cases 的 local_job_id 读取，已持久化任务可在重启后查询；原始 trace 不通过 MCP 返回。';
const redact=redactLocal;
export class NativeCli {
 constructor({spawnProcess=spawn,engineRoot,engineSha,artifactRoot=path.join(homedir(),'panqu-remote-reports','local')}={}){if(!path.isAbsolute(engineRoot??'')||!/^[a-f0-9]{40}$/.test(engineSha??''))throw Error('PINNED_ENGINE_REQUIRED');this.engineSha=engineSha;this.jobs=new Map();this.spawn=spawnProcess;this.engineRoot=engineRoot;this.artifactRoot=artifactRoot;this.store=new JobStore(artifactRoot);}
 snapshot(j){return {jobId:j.id,program:j.program,state:j.state,exitCode:j.exitCode??null,signal:j.signal??null,output:redact(j.output),truncated:j.truncated,executionLocation:'LOCAL_MAC',engineSha:j.engineSha,...(j.restored?{restored:true,note:'已恢复任务索引；不恢复历史 stdout、不自动重跑。UNKNOWN_AFTER_RESTART 表示未确认终态。'}:{}),...(j.persistenceError?{persistenceError:j.persistenceError}:{}),...(j.artifactDirectory?{report:{tool:'panqu_report',local_job_id:j.id,directory:j.artifactDirectory}}:{})};}
 async job(id){return this.jobs.get(id)??this.store.load(id);}
 async report(id,args={}){return localReport(await this.job(id),args);}
 async cases(id,args={}){return localCases(await this.job(id),args);}
 async invoke(input){
  ...
  if(input.action==='status')return this.snapshot(await this.job(input.job_id));
  ...
  if([...this.jobs.values()].some(j=>j.state==='RUNNING'||j.child))throw Error('LOCAL_JOB_ALREADY_RUNNING');
  ...
  const pw=await import(pathToFileURL(path.join(this.engineRoot,'dist/src/devtest/run-playwright-cli.js')).href);
  const flowPlaywright=input.program==='devtest'&&pw.isLegacyPlaywrightCommand(args);
  const businessSuite=input.program==='devtest'&&(args[0]==='business-suite'||(args[0]==='flow'&&args[1]==='business-suite'));
  const realFlow=input.program==='devtest'&&(['real-video','real-image','real-canvas'].includes(args[0])||(args[0]==='flow'&&['real-video-submit','real-image-submit','real-canvas-submit'].includes(args[1])));
  const reportable=playwright||flowPlaywright||businessSuite||realFlow;
  const help=args.includes('--help')||args.includes('-h');
  let mock=false;
  if(playwright&&!help){
   mock=pw.parseCliArgs(input.program==='playwright'?args:args.slice(1)).isMock;
  }
  if(flowPlaywright&&!help)mock=pw.parseCliArgs(pw.normalizeLegacyPlaywrightArgs(args.slice(2))).isMock;
  if(reportable&&!help&&!mock&&input.confirm_real_execution!==true)throw Error('REAL_EXECUTION_CONFIRMATION_REQUIRED');
  const id=randomUUID(),j={id,program:input.program,state:'RUNNING',output:'',truncated:false,engineSha:this.engineSha,createdAt:Date.now()};
  if(reportable&&!help){
  ...
  this.jobs.set(id,j);
  let child;
  try{child=this.spawn(process.execPath,[script,...args],{cwd,shell:false,detached:true,stdio:['ignore','pipe','pipe'],env});j.child=child;}
  catch{j.state='START_FAILED';await this.store.save(j);throw Error('LOCAL_START_FAILED');}
  ...
  child.on('close',async(code,signal)=>{
   clearTimeout(timer);clearTimeout(j.kill);
   const finalState=j.state==='RUNNING'?(code===0?'COMPLETED':'EXITED_NONZERO'):j.state;
   try{await this.store.save({...j,state:finalState,exitCode:code,signal});}catch{j.persistenceError='JOB_INDEX_WRITE_FAILED';}
   j.state=finalState;j.exitCode=code;j.signal=signal;delete j.child;
  });
