...
- 新增统一执行结果契约、DevTest 覆盖台账和 Acceptance / Agent / Platform 适配模块，生命周期与业务断言分离。
- 报告及 MCP 摘要区分产品缺陷、测试阻断、未测试和通过项，增加数据使用及统计对账；发现候选不计入正式计划。
- 执行提示区分全局与单用例阻断，保留计划绑定、执行确认和幂等控制。
- 修复兼容标题与 Markdown 表格，收紧媒体、画布、门禁报告的证据范围描述。
- 重组 README 导航、快速开始、能力、入口、状态、报告、安全与发布核验；白名单显式纳入六个新运行时模块。
- 本次不声明真实供应商生成、真实账务或全平台契约迁移已验收，不发布 npm。
...
- 导出测试数据生命周期管理器与需求溯源矩阵构建器；两者为独立接口，不宣称主流程已自动登记/清理所有业务实体或自动输出矩阵附件。
- 修正新增测试与现有类型契约不一致的问题；安装包明确列出三个 MCP 运行时文件，排除 MCP 测试文件，核对四个新增模块的 JS/声明文件。
- README 严格区分主流程集成、独立接口、证据缺口、受控测试与真实业务验收；GitHub、本地 MCP 和云端入口按固定提交验收。
- 延续真实执行授权、凭据隔离和历史任务 SHA 保留规则；不发布 npm，不自动新增 AI/数据库配置。
- 修复安装包缺失的 MCP 版本依赖；移除机器专属会话默认路径，源码快照检查改为显式选择，Skill 命令示例使用通用路径。
...
- 图片、视频、画布专项与主 Skill 串联代码入口、输入/状态边界、本地行为、API、页面和真实业务结果，安全本地测试作为独立补充，不替代 MCP 计划。
- 主入口修正“禁止替代内核用例”被误解为“禁止已有本地测试”的流程冲突；真实测试名/命令/源码版本/退出码和证据层级必须记录，缺依赖不盲装，未知规则不自行补全。
- 沿用固定七章单表及两份同源用例文档，增加写入前结构及跨文件一致性门禁；公开 validateDeveloperHandoffMarkdown 供补充报告复用，不修改历史报告或伪造缺失证据。
- 未运行的 Panqu 源码回归候选进入报告缺口，不因页面通过消失；候选不自动成为用户确认的产品要求。
- 增加反例回归测试；打包增加校验模块及两份引用规范，共四项资产，文件数上限 304 并显式核对新增项，其余安全边界不变。

## [4.32.0] - 2026-09-09

### Panqu 业务证据协调

- 任务、媒体和最终结算作为三个独立持久事实；下一步从缺失证据派生，结算延迟只补查账单，不重提任务或重复下载成功验证的媒体。
- 失败任务保留已知失败并进入对账阶段；可明确记录扣款、退款和最终净扣款，不从“任务失败”推断免费或已退款。账单暂时不可用不再丢失任务/媒体事实。
- Nuxt 任务关联 requestId/目标节点，旧源时间戳观察不覆盖新事实，冲突终态保留并停止自动读取。首次生成前缺失最终结算契约即阻断；历史终态只保留历史结论，不自动升级。
- 已提交任务的定向观察与首次生成预检拆分；前端源码/原素材更新不阻塞对账，但原计划、任务、驱动身份、Origin 和有效审批仍严格绑定。
- 39 项专项及独立 HTTP/FFmpeg/CLI 故障注入通过；不代表真实供应商生成或账单验收。实际账单终态接口及任务命名空间仍须操作员确认，不能从源码或模型猜测。
- 全量检查中发现的既有 DNS 20ms 预算、Tracer 10ms 测试改用可控时钟，保留/强化原断言，不改生产实现；新增固定来源时间戳按项目 FIXED_ISO 测试规范显式注入。
- 打包增加一个运行时模块的 JS/声明文件，文件数上限从小于300调整为最多300，并逐项验证新增文件存在；原允许目录、体积和敏感文件门禁不变。
...
- 新增 `devtest mission plan/run/resume/status/materials/prepare-media`，以结构化需求、可信合法组合和当前报价编译最低成本方案；模型建议不能改变目标模型、预算、授权或测试结论。
- 单任务持久化状态机在提交前保存意图；并发锁、进程恢复、未知提交隔离及最多一次提交防止断线或重启导致重复生成。审批过期、源码/素材变化、参数或身份不符时阻断。
- 按项目实现 PHP CSRF/FormData 视频协议与 Nuxt 单节点 JSON 画布协议；通过同一内核恢复原任务、核对任务身份、实际解码媒体和任务级账单。缺失证据不报 PASS，超额扣费和媒体尺寸/时长冲突保留失败证据。
- 新增指定目录素材检查、有限测试文案准备和小型参考视频合成；禁止跳转及应用凭证转发。参考素材上传、自动能力/价格采集、全局账户预算与语义质量验收仍未实现，不由模型补猜。
- 29 项专项回归包含独立本地 HTTP、真实媒体解码、跨 CLI 进程恢复和故障注入；不代表客户业务环境或真实供应商生成验收。既有只读 MCP 门禁不放宽。
...
- DevTest 去重改为结构化精确比较，保留大小写、正负号、标点、值类型、数组顺序/重复项、Header、Path、断言及契约指纹差异；仅允许 ID、展示名称和优先级不同的等价用例合并，不修改调用方数据。
- 数值/布尔输入按完整显式契约校验：补齐 true/false 与数值/布尔枚举正反例，整数边界按整数域取整，修正负上限与无合法整数区间的基线；小数负例必须满足其他范围/枚举约束，Query 布尔负例不能序列化成合法值。
- 正例与完整约束冲突、负例同时违反其他约束、无法构造单故障样本时保留 DESIGNED_ONLY 及原因；无法构造的小数/枚举负例不再污染“字段缺失”用例。未确认预期、不可表达输入及写入副作用证据门禁继续保留。
- 已确认、有来源且有明确预期的基础边界进入默认 Tier 1，仍受用例预算、维度开关和安全门禁约束；未知规则与通用极端数值不自动提升。MCP 绑定新执行策略，旧计划需要重新规划和确认。
- 新增参数组合、精确去重、默认选择和独立 HTTP 注入缺陷回归；覆盖上限多放行 1、大小写/负号损坏及健康对照，不将受控测试结果宣传为真实业务环境验收。

### 输入参数与默认值检查清单

- 画布、视频和图片专项新增独立输入检查表，明确提取格式、单个/整批大小、单次/累计数量、提示词条数/计数单位、默认值时机、联动和违规副作用；每条规则保留需求来源，缺失/冲突不能拿常识或代码补成已确认。
- 按确认的单位、范围与精度检查上下界、组合超限、切模型及旧值恢复，关联实际计划用例和证据；不把 Skill 指导宣传为产品运行时拦截或新增上传/生成执行能力。
- 安装和 npm 包携带输入参考文件，升级补齐缺失文件并保留团队修改；新增升级补齐与输入清单符号链接隔离回归。

### Panqu 功能专项 Skills

- 新增画布、视频模型、图片模型三个独立 Skill 与源码索引，区分 Nuxt/Vue Flow、React/XYFlow 及 PHP/Go 宿主；按本次功能变化核对组件兼容、模型能力、参数、异步任务、结果和结算，不固化模型名称名单。
- 主 DevTest Skill 在计划前按功能语义组合读取专项指导；继续遵守一次确认、SAFE 执行和需求证据门禁，不把源码检查或离线通过当作真实生成验收。
- 构建、npm 包和 `init --trae` 安装完整 Skill 资源；保留团队编辑，逐级拒绝符号链接越界。新增资源一致性、重复安装与路径隔离回归。自动选择依赖宿主和模型，安装测试不代表 Trae 模型已通过行为评测。

### Trae 确认后执行流程

- 将薄入口 Skill 改为中文状态分支：定位需求、集中澄清、一次确认、立即调用执行、异常查询和证据交付；明确不猜规则、不重复确认、不擅自扩展授权。
- MCP 新增确定性 `next_action`，返回需要澄清的问题或可直接使用的计划执行/查询参数与稳定幂等键，降低入口模型自行拼接参数和恢复状态的负担；不替代真实用户授权，不放宽现有内核门禁。

### 每次执行强制需求闭环

- 新增 `NO_SILENT_REQUIREMENT_GAPS_V1` 内核硬门禁：逐项保留原文、来源、用例关联和待澄清问题，并交叉核对原文未进入账本的行。
- 未理解、待确认、推测和未知来源的业务规则不能作为预期；相关用例在 Processor、Scenario 和数据准备前阻断，未知影响无法限定时阻断全部所选用例。
- 执行后从完整需求与全部候选用例复核，未覆盖、未选择、未执行或证据不足的要求不得计为通过；重跑不能借用旧 PASS 抹去本轮缺口。明确且不受影响的用例仍可执行。
- CLI、MCP、状态查询和交付产物保留同一门禁；旧策略计划必须重建。Trae Skill 明确区分执行授权与业务确认，只使用用户实际确认的答案更新需求并重新规划。
- 验证覆盖真实 HTTP、执行前阻断、遗漏原文、未覆盖断言、部分重跑及伪 PASS；将旧通知分发单元测试改为注入确定性传输失败，避免外部 DNS 超时影响回归，不改通知生产逻辑。

### 缺陷检测与自动复核

- 修复完整 JSON 缺字段被判 UNKNOWN、业务错误值中的 network/timeout 被误归环境故障的漏检；保留 5xx、未观察响应等不确定性门禁。
- SAFE 内置单步骤只读失败最多自动复核一次，复用原超时和取消信号，计划预算包含额外请求；不自动重试写操作、多步骤/状态流程、自定义 Processor 或快照观察器。
- 保留两次证据：稳定重复失败才可证明可复现；不一致/复核失败不变成 PASS，也不直接确认产品 Bug。
- 最小复现直接给出字段路径、预期、实际与缺失标记；按接口与断言预期保守聚合，避免无关缺陷被一个通用根因吞并。
- 修复内置 HTTP Scenario 适配时丢失实际 Binding Gate 证据、导致正常请求被错误阻断的问题；只传递原执行器的已验证绑定，不伪造 Gate PASS。
- 新增独立 HTTP 注入缺陷、正常样本、环境异常与不稳定读取回归；这些是开发可见的受控回归，不是生产缺陷命中率或人工耗时测量。未增加报表/人工计时产品功能。
