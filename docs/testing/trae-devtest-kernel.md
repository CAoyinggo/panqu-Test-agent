...
- 去重保留实际输入和断言的大小写、符号、类型、数组顺序/重复项、Header/Path、契约指纹及证据要求；相似描述不是同一用例。
- 数值/布尔正例必须同时满足类型、范围、枚举和可空约束。整数区间没有合法值、边界被枚举排除时不能期待成功；保留 DESIGNED_ONLY 及原因。
- 数值/布尔负例只放松目标约束后必须满足其他显式约束，才可证明目标规则的独立校验；构造失败保留 `SINGLE_FAULT_VECTOR_UNAVAILABLE`。它是测试设计缺口，不是产品 Bug，也不是 PASS。搜索有界，未找到不等于数学证明不存在。
- 布尔值同时覆盖 true/false；Query/Header/Path 中不能将合法字符串 `false` 当作类型错误，也不能将省略或字符串 `null` 当作 JSON null。
- 已确认、可追溯且预期明确的基础边界默认进入 Tier 1；用例预算截断、维度禁用、缺少 Observer 或安全策略拒绝仍形成可见缺口。默认选择不等于授权写入或付费调用。
