...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  if (argv[0] === 'playwright' || argv[0] === 'flow') {
    const pw = await import('../src/devtest/run-playwright-cli.js');
    if (argv[0] === 'playwright') return pw.main(argv.slice(1));
    if (pw.isLegacyPlaywrightCommand(argv)) return pw.main(pw.normalizeLegacyPlaywrightArgs(argv.slice(2)));
  }
  ...
  try {
    ...
    if (command === 'init') {
      ...
      if (rest.includes('--trae')) result.created.push(...await initializeDevTestTrae(root));
