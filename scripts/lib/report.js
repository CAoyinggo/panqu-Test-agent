// HTML 可视化报告生成（自包含、无外部依赖，遵循 html-report 视觉规范）
const fs = require('fs');
...
function buildReport(data) {
  ...
  // 概览指标
  const overviewCards = [
    { label: '任务状态', value: esc(d.submit && d.submit.status || '-') },
    { label: '任务 ID', value: esc(d.submit && d.submit.taskId || '-') },
    { label: '模型', value: esc(d.taskDef && d.taskDef.model_name || '-') },
  ...
  // 用例表格
  const casesRows = (d.cases || []).map(c => [
    esc(c.id), esc(c.name), badge(c.result), esc(c.detail || '-'),
  ]);

  // 接口响应摘要
  const respRows = (d.responses || []).map(r => [
    esc(r.name), esc(r.method), esc(r.status), esc(r.code != null ? r.code : '-'),
    '<code>' + esc(String(r.summary || '').slice(0, 160)) + '</code>',
  ]);

  // 数据影响清单
  const impactRows = (d.impact || []).map(i => [
    esc(i.type), esc(i.name), esc(i.action), esc(i.desc),
  ]);

  // 数据隔离核验
  const checkRows = (d.checks || []).map(c => [
    esc(c.name), c.pass ? badge('PASS') : badge('FAIL'), esc(c.detail || '-'),
  ]);

  // 浏览器人工待办
  const manualRows = (d.manual || []).map(m => [
    esc(m.id), esc(m.steps || '-'),
  ]);

  // 问题卡点
  const issueRows = (d.issues || []).map(i => [
    esc(i.level), esc(i.title), esc(i.desc || ''),
  ]);

  const issueHtml = issueRows.length
    ? tableHtml(['级别', '问题', '说明'], issueRows)
    : '<p class="muted">无</p>';

  const manualHtml = manualRows.length
    ? tableHtml(['用例', '人工操作步骤'], manualRows)
    : '<p class="muted">本次无浏览器人工待办</p>';
  ...
  if (ai.exists) {
    ...
    const resolvedRows = (ai.resolved || []).map(r => [
      esc(r.field), '<code>' + esc(r.path) + '</code>', esc(r.full || '⚠ 未找到'),
    ]);
    const resolvedHtml = resolvedRows.length
      ? tableHtml(['字段', '相对路径', '解析结果'], resolvedRows)
      : '<p class="muted">本任务未引用素材库文件</p>';
...
<div class="wrap">
  <header class="cover">
    ...
    <div class="sub">${esc(d.taskDef && d.taskDef.scene || '')} ｜ ${esc(d.env)} 环境 ｜ 生成于 ${esc(time)}</div>
    <div class="meta">
      <span>任务名：${esc(d.taskDef && d.taskDef.name || '-')}</span>
      <span>项目 ID：${esc(d.taskDef && d.taskDef.project_id || '-')}</span>
      <span>账号：${esc(d.taskDef && d.taskDef.account || '-')}</span>
    </div>
  </header>

  <h2>一、执行概览</h2>
  <div class="cards">
    ${overviewCards.map(c => `<div class="card"><div class="v">${c.value}</div><div class="l">${c.label}</div></div>`).join('')}
  </div>
  ${d.submit && d.submit.err ? `<div class="callout"><strong>任务失败原因：</strong>${esc(d.submit.err)}</div>` : ''}
  ...
  <footer>
    <p>由 test-flow 一键执行脚本自动生成 ｜ 数据来源：test.panqu.com API 实测</p>
  </footer>
</div>
</body>
</html>
...
// 写报告文件，返回路径
function writeReport(outputDir, slugBase, data) {
...
module.exports = { buildReport, writeReport, badge, esc, tableHtml };
