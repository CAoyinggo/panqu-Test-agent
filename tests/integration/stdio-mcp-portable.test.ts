...
describe('stdio MCP：仓库内可移植性', () => {
  ...
  it('项目入口迁移到 DevTest 内核，旧 stdio 仅保留为兼容测试', () => {
    ...
    expect(Object.keys(mcpServers)).toEqual(['devtest']);
    const cfg = mcpServers.devtest;
    // 拒绝 server 级未知字段漂移：config 键严格只有 args/command/env。
    expect(Object.keys(cfg).sort()).toEqual(['args', 'command']);
    expect(cfg.command).toBe('node');
    expect(cfg.args).toEqual(['${workspaceFolder}/dist/bin/devtest-mcp.js', '--project-root', '${workspaceFolder}']);
  });
