...
describe('Phase 52 Cost Dashboard E2E', () => {
  it('Dashboard API 提供全部成本、趋势、预测、预算、异常与容量数据', async () => {
    ...
    cost.ledger.record({ projectId: 'wan3', runId: 'r1', evaluationId: 'e1', benchmarkId: 'b1', provider: 'openai', model: 'cheap', category: 'LLM', quantity: 10, unitCost: 0.1, currency: 'USD', timestamp: '2026-08-21T01:00:00Z' });
    cost.setBudget({ projectId: 'wan3', monthly: 200 }, 'admin');
    server = createPlatformServer({ service: bundle.service, auth: bundle.auth, mode: 'test', costGovernance: cost, now: () => '2026-08-21T02:00:00Z' }); const { url } = await server.listen();
    ...
    expect(summary.trend).toEqual([{ period: '2026-08-21', cost: 1 }]);
