...
/** Synthetic contract fixture. Values are derived from assertions: this proves adapter wiring ONLY,
 * never business behavior or a real environment PASS. See devtest-mcp-kernel.test.ts for independent HTTP evidence. */
...
describe('P0 three simulated Reference Scenario contracts (NOT real business acceptance)', () => {
    ...
    it(`${reference.id}: Generator + Quality Gate 原始 Case 直接进入 Runtime/Runner`, async () => {
      ...
      if (process.env.DEVTEST_REFERENCE_METRICS === '1') {
        process.stdout.write(`[reference-metrics] ${JSON.stringify({
          validationLevel: 'SIMULATED_CONTRACT_ONLY',
          id: reference.id,
