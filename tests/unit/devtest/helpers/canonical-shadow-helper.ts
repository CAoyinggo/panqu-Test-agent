/**
 * Panqu AI DevTest — Canonical Shadow Comparison Helper
 * Phase 1.5B 真实执行旧版 verify 的离线 Shadow Comparison 辅助工具
 *
 * 核心架构边界 (遵守 docs/ARCHITECTURE_FREEZE.md 受控扩展原则):
 * 1. 本模块纯粹存在于测试套件中，严禁从 core-kernel、CLI、MCP 或任何生产调用链中导入；
 * 2. 真实调用当前 core-kernel.verify() 获取实际 VerifyKernelResult，严禁在主测试场景中手工构造或预填旧版裁决；
 * 3. 显式归一化旧版裁决为 PASS / FAIL / UNVERIFIED；
...
export type NormalizedLegacyVerdict = 'PASS' | 'FAIL' | 'UNVERIFIED';
...
/**
 * 将旧版 VerifyKernelResult 显式归一化为三态裁决：
 * - acceptance === 'REJECTED' || verdict === 'FAIL' || status in ['FAILED', 'ERROR'] -> 'FAIL'
 * - verdict === 'PASS' && acceptance === 'ACCEPTED' && passed === true -> 'PASS'
 * - 其余 (PROCESSING, BLOCKED, UNVERIFIED) -> 'UNVERIFIED'
 */
export function normalizeLegacyVerdict(legacy: VerifyKernelResult): NormalizedLegacyVerdict {
  if (
    legacy.acceptance === 'REJECTED' ||
    legacy.verdict === 'FAIL' ||
 ...
 *     3. legacyNormalized === 'PASS';
 *     4. canonicalVerdict === 'FAIL' || canonicalVerdict === 'UNVERIFIED'.
 *   未显式标记或原因码不在白名单的收紧，必须分类为 NEEDS_REVIEW！
 * - REGRESSION_RISK: 旧版为 FAIL/UNVERIFIED，新引擎判定为 PASS（冒进放行）
 * - MAPPING_GAP: 旧数据映射失败 (mappingSuccess === false)
 * - SEMANTIC_MISMATCH: 语义不可比
 * - NEEDS_REVIEW: 无法自动判定的差异
 */
export function classifyShadowDifference(params: {
  legacyNormalized: NormalizedLegacyVerdict;
  ...
  if (params.legacyNormalized === params.canonicalVerdict) {
    return 'MATCH';
  }
  // 旧版为 PASS，新版为 FAIL 或 UNVERIFIED
  if (
    params.legacyNormalized === 'PASS' &&
  ...
  // 旧版为 FAIL 或 UNVERIFIED，新版判定为 PASS (更宽松，冒进漏检风险)
  if (
    (params.legacyNormalized === 'FAIL' || params.legacyNormalized === 'UNVERIFIED') &&
...
export interface ShadowTestCase {
  ...
  testId: string;
  capturedAt: string;
  verifyOptions: VerifyKernelOptions; // 必须实际调用 verify()
  spec: CanonicalTestSpec;
  legacyNormalizedVerdict?: NormalizedLegacyVerdict; // 历史基准（表驱动数据，禁止在测试中复制旧版 verify 源码）
...
export interface ShadowComparisonRecord {
  ...
  testId: string;
  tags: string[];
  legacyVerdict: NormalizedLegacyVerdict;
  rawLegacyVerdict: string;
  rawLegacyAcceptance?: string;
  canonicalVerdict: CanonicalVerdict | 'MAPPING_FAILED';
...
/**
 * 执行单个影子比对用例（实际调用 core-kernel.verify() 并结合表驱动基准比对）
 */
export async function runSingleShadowComparison(
  testCase: ShadowTestCase
): Promise<ShadowComparisonRecord> {
  // 1. 实际执行当前 core-kernel.verify()
  const actualVerifyResult = await verify(testCase.verifyOptions);

  // 2. 归一化旧版裁决：优先来自表驱动固化测试基准（避免保留第二套旧版裁决引擎源码）
  const legacyNormalized = testCase.legacyNormalizedVerdict ?? normalizeLegacyVerdict(actualVerifyResult);
  ...
  return {
    ...
    testId: testCase.testId,
    tags: testCase.tags || [],
    legacyVerdict: legacyNormalized,
